...
char *apfloat_to_string(APFloat *num, enum PrintType print_type) {
    ...
    if (print_type == BASE10) scientific_notation = 1;
        ...
        scientific_notation = num->significand->size + num->exponent > 6 || num->significand->size + num->exponent < -4;
    }
    ...
    if (scientific_notation && print_type != BASE10) {
        ...
        // e takes up 1 byte of space (assuming an extra space before the multiplication sign).
        // We also have to allocate space for the exponent.
        // Decimal point also takes up one byte of space.
        int extra_space = 3 + get_exponent_digits(new_exponent);
        if (num->sign == 1) extra_space++;
        char *res = (char*)malloc(num->significand->size * sizeof(char) + extra_space);
        char *after_decimal = (char*)malloc(num->significand->size * sizeof(char));
        ...
        after_decimal[num->significand->size - 1] = '\0';

        // Print the result into res
        snprintf(res, num->significand->size + extra_space, "%d.%se%+lld", num->significand->digits[num->significand->size-1], after_decimal, new_exponent);
        ...
        // Print the result into res
        snprintf(res, num->significand->size + extra_space, "%d.%s * 10^%lld", num->significand->digits[num->significand->size-1], after_decimal, new_exponent);
